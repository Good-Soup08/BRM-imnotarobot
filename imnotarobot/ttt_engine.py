import numpy as np
import random
from app import Gestures # ver se dá bem com ele aqui
...
class TicTacToe:
    ...
    def show(self): # para motivos de teste apenas
    ...
    def main(self, draw_grid): 
        ...
        while (True):
            ...
            while (not (int(inp) >= 1 and int(inp) <= 9)):
                ...
                for index in range(np.size(self.selected_numbers)):
                    if (self.selected_numbers[index] == gesture_number):
                        inp = index + 1
                        break
                if (gesture_number == -1):  # -1 para quando nao recolhe nenhum gesto no tempo
            ...
            #remoção das 2 seguintes tb já é 
            ...
            # falta meter o robô na posição inicial no fim de cada jogada
            # falta desligar robot qd jogo termina...?
            # falta parte de dar q jogo termine a meio por vontade dos jogadores
